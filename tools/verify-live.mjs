...
const url = 'https://smorchj.github.io/Belves-Scroll/?skip=1';
...
const failed = [];
page.on('requestfailed', (r) => failed.push(r.url().replace('http://smorchj.github.io', '')));
page.on('response', (r) => { if (r.status() === 404) failed.push('404 ' + r.url().replace('http://smorchj.github.io', '')); });
